...
export default function NFCIndex() {
	...
	async function read_nfc() {
		...
		try {
			...
			const consulta = await fetchData(
				`/api/pulseras/uuid/${encodeURIComponent(uid)}`,
			);
			...
			if (pulseras.length === 0 && !consulta.ok) {
				...
				if (response.ok) {
					Alert.alert(
						'Pulsera creada',
						'Pulsera asociada correctamente',
					);
				} else {
		...
		<View style={styles.container}>
			<View style={styles.view_separator}>
				{scanning ? (
					...
					<View style={styles.view_card}>
						<Text style={styles.text}>NO HAY PULSERA ASOCIADA</Text>
						<Image
