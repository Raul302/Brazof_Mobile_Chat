...
function InboxIndex() {
	...
	async function formatearChats(chats) {
		const lista = await Promise.all(
			chats.map(async (chat) => {
				...
				try {
					...
					const mensajes = await fetchData(
						`/api/mensajes/chat/${id}?chat_id=${id}`,
					);
